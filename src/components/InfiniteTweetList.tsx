...
const InfiniteTweetList: React.FC<InfiniteTweetProps> = ({
    tweets,
    isError,
    isLoading,
    hasMore = false,
...
function TweetCard({ id, user, content, createdAt, likeCount, likedByMe }: Tweet) {
    ...
    const toggleLike = api.tweet.toggleLike.useMutation({
        onSuccess:
            ({ addedLike }) => {
                const updateData: Parameters<typeof trpsUtils.tweet.infiniteFeed.setInfiniteData>[1] = (oldData) => {
                ...
                //await trpsUtils.tweet.infiniteFeed.invalidate();
                trpsUtils.tweet.infiniteFeed.setInfiniteData({}, updateData);
                trpsUtils.tweet.infiniteFeed.setInfiniteData({ onlyFollowing: true }, updateData);
                trpsUtils.tweet.infiniteProfileFeed.setInfiniteData({ userId: user.id }, updateData);
            }
    });
...
function HeartButton({ isLoading, onClick, likedByMe, likeCount }: HeartButtonProps) {
    ...
    return <button disabled={isLoading} onClick={onClick} className={`group -ml-2 items-center gap-1 self-start flex transition-colors duration-200 ${likedByMe
        ? "text-red-500"
